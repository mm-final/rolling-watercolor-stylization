...
class HashTablePermutohedral(object):
    def __init__(self, kd_, vd_):
        ...
        self.capacity = 2 ** 15
        self.filled = 0
        self.entries = [{'key_idx': -1, 'value_idx': -1}
    ...
    def _grow(self):
        ...
        new_entries = [{'key_idx': -1, 'value_idx': -1}
                       for _ in range(self.capacity)]
...
class PermutohedralLattice(object):
    ...
    def __init__(self, d, vd, inp_len):
        ...
        self.replay = [{'offset': 0, 'weight': 0.}
                       for _ in range(inp_len * self.d1)]
        self.nReplay = 0
    ...
    def filter(inp, ref, debug=True):
        ...
        lattice = PermutohedralLattice(
            ref.shape[-1], inp.shape[-1] + 1, inp.shape[0] * inp.shape[1])
        ...
        if debug:
            ...
            for i in range(len(names)):
                log.info('{}: {}s'.format(
                    names[i], run_times[i + 1] - run_times[i]))
    ...
    def splat(self, position, pos_idx, value):
        ...
        self.elevated[self.d] = -self.d * position[pos_idx +
                                                   self.d - 1] * self.scale_factor[self.d - 1]
        ...
        self.elevated[0] = self.elevated[1] + 2 * \
            position[pos_idx] * self.scale_factor[0]

        v = self.elevated * self.splat_scale
        v_ceil = np.ceil(v) * self.d1
        v_floor = np.floor(v) * self.d1
        self.greedy = np.where(
            v_ceil - self.elevated < self.elevated - v_floor, v_ceil, v_floor).astype('int16')
        ...
        for i in range(self.d1):
            # print(self.d - self.rank[i])
            self.barycentric[int(self.d - self.rank[i])] += t[i]
            self.barycentric[int(self.d1 - self.rank[i])] -= t[i]
        ...
        for remainder in range(self.d1):
            ...
            #                                                                                           :self.vd]
            self.hash_table.values[hash_idx:hash_idx +
                                   self.vd] += self.barycentric[remainder] * value[:self.vd]
    ...
    def blur(self, reverse=False):
        ...
        new_vals = np.zeros(
            (self.vd * self.hash_table.size()), dtype='float64')
        ...
        for j in r:
            ...
            for i in range(self.hash_table.size()):  # blur point i in dimension j
                ...
                # keys to the neighbors along the given axis.
                neighbour2[j] = key[j + i * self.d] + self.d

                old_val_offset = old_vals_idx + i * self.vd
                new_val_offset = new_vals_idx + i * self.vd

                vm1_idx = self.hash_table.lookup(
                    neighbour1, False)  # look up first neighbor
                if vm1_idx is not None:
                    vm1_idx -= hash_table_base_idx + old_vals_idx
                else:
                    vm1_idx = None

                vp1_idx = self.hash_table.lookup(
                    neighbour2, False)  # look up second neighbor
